...
from ledger.schema.events import AgentInputValidated, AgentInputValidationFailed
...
class BaseApexAgent(ABC):
    ...
    async def _record_node_execution(self, name, in_keys, out_keys, ms, tok_in=None, tok_out=None, cost=None):
        ...
        await self._append_session({"event_type":"AgentNodeExecuted","event_version":1,"payload":{
            ...
            # Both token counts must be set to count as an LLM call (avoid tok_in=0 alone).
            "llm_called": tok_in is not None and tok_out is not None,
            "llm_tokens_input":tok_in,"llm_tokens_output":tok_out,
    ...
    async def _complete_session(self, result):
        ...
        next_agent = result.get("next_agent_triggered") or result.get("next_agent")
        await self._append_session({"event_type":"AgentSessionCompleted","event_version":1,"payload":{
            ...
            "next_agent_triggered":next_agent,"completed_at":datetime.now().isoformat()}})
        _LOG.info(
            "agent_run_complete agent_type=%s session_id=%s application_id=%s duration_ms=%s nodes=%s next=%s",
    ...
    async def _append_session(self, event: dict):
        ...
        _LOG.info(
            "session_event agent_type=%s session_id=%s event_type=%s node=%s llm_called=%s",
            self.agent_type,
            self.session_id,
            et,
            pl.get("node_name"),
    ...
    async def _append_stream(self, stream_id: str, event_dict: dict, causation_id: str = None):
        ...
        _LOG.info(
            "stream_append stream_id=%s event_type=%s causation_id=%s document_id=%s",
            stream_id,
            et,
            causation_id,
            payload.get("document_id"),
        )
        _LOG.debug("stream_append_payload=%s", json.dumps(payload, default=str)[:1500])
        for attempt in range(MAX_OCC_RETRIES):
            try:
                ...
                pos = ver + 1 if ver >= 0 else 0
                _LOG.debug("stream_append_ok stream_id=%s event_type=%s position=%s attempt=%s", stream_id, et, pos, attempt)
                return
            except Exception as e:
                _LOG.warning(
                    "stream_append_retry stream_id=%s event_type=%s attempt=%s error=%s",
                    stream_id, et, attempt, type(e).__name__,
                )
                if "OptimisticConcurrencyError" in type(e).__name__ and attempt < MAX_OCC_RETRIES-1:
    ...
    async def _append_with_retry(
        ...
        """Append one or more events atomically with OCC retry; returns assigned stream positions."""
        if not events:
            return []
        et_summary = events[0].get("event_type", "?")
        _LOG.info(
            "stream_append_batch stream_id=%s first_event=%s count=%s causation_id=%s",
            stream_id,
            et_summary,
            len(events),
            causation_id,
        ...
        for attempt in range(MAX_OCC_RETRIES):
            try:
                ...
                _LOG.debug(
                    "stream_append_batch_ok stream_id=%s positions=%s attempt=%s",
                    stream_id,
                    positions,
                    attempt,
                )
                ...
                _LOG.warning(
                    "stream_append_batch_retry stream_id=%s attempt=%s error=%s",
                    stream_id,
                    attempt,
                    type(e).__name__,
                )
    ...
    async def _record_input_validated(self, inputs_validated: list[str], validation_duration_ms: int) -> None:
    ...
    async def _call_llm(self, system, user, max_tokens=1024):
        ...
        cost = getattr(resp.usage, "cost_usd", None)
        if cost is None:
            cost = round(i / 1e6 * 3.0 + o / 1e6 * 15.0, 6)  # Claude-scale fallback
        return t, i, o, float(cost)
